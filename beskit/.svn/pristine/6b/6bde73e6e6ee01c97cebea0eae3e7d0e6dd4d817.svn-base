...
public class UserController {
	...
	public Map<String, String> insertPic(@ModelAttribute FileVO fileVO, HttpSession session) throws IOException {
        ...
        file.delete();
        map.put("url", "./inputPic/thumb_" + memberVO.getMem_no() + "_" + imageName);
	...
	public ModelAndView updateFinal(@ModelAttribute MemberVO memberVO, @RequestParam String phone1,
		...
		if(memberVO.getMem_country() == 0) memberVO.setMem_phone(phone1 + "-" + phone2 + "-" + phone3);
		else memberVO.setMem_phone(phone1 + phone2 + phone3);
	...
	public ModelAndView leave(HttpSession session){
		...
		ModelAndView mav = new ModelAndView("user/leave");
		mav.addObject("memberVO", memberVO);
	...
	@RequestMapping(value="leave", method=RequestMethod.POST,
			...
			produces={MediaType.APPLICATION_JSON_UTF8_VALUE})
	@ResponseBody
	public void leaveAction(@RequestParam long mem_no, HttpSession session) throws Exception{
		userService.leaveAccount(mem_no);
