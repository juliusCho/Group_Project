	...
	function articleList(moreArtBtnNo) {
		$.ajax({
			...
			if(list.length == 0) {
				...
				articleList += '<h2 id="noArticleMsg"><fmt:message key="square.noArtMsg"/></h2>';	
				...
				$.each(list, function(key,val) {
					...
					if(val['mem_no'] == '${sessionScope.memberInfo.mem_no}') {
						...
						articleList += '<input type="button" value="<fmt:message key="home.update"/>" onclick="javascript:updateArticle(' + val['art_no'] + ');"/>';
						articleList += '<input type="button" value="<fmt:message key="home.delete"/>" onclick="javascript:deleteArticle(' + val['art_no'] + ');"/>';
						articleList += '</td></tr>';
					}
