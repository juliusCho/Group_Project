...
<%@ include file="/resources/jsp/script/chat.jsp" %>
...
// 왼쪽메뉴 제어
	...
	function deleteWaitingFriendDirect(waiting_fri_no){
		...
		if(value){
			$.ajax({
				method:'GET',
				url:'../../friend/${sessionScope.memberInfo.mem_no}/deleteWaitingFriend',
	...
	function acceptFriend(waiting_fri_no){
	 	$.ajax({
			method:'GET',
			url:'../../friend/${sessionScope.memberInfo.mem_no}/acceptFriend',
			data:{waiting_fri_no:waiting_fri_no}
		}).done(function(data){
			if(data==2){
...
// 회원페이지 출력
	...
	$(function() {
		...
		$('#art_content').keyup(function() {
			...
			if(textCount > 600) {
				alert('<fmt:message key="home.artLength"/>');
	...
	function articleList(moreArtBtnNo) {
      $.ajax({
         ...
         if(list.length == 0) {
            ...
            articleList += '<h2 id="noArticleMsg"><fmt:message key="mypage.noArtMsg"/></h2>';   
	...
	function commentList(art_no, moreComBtnNo) {
		$.ajax({
			...
			if(20 <= comment.length && comment.length % 20 == 0) {
				moreComBtnNo++;
				$('#moreComBtn').html('<input type="button" value="<fmt:message key="home.seemore"/>" align="center"' + 
